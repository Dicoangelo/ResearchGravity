...
Ported from jacob-bd/notebooklm-mcp-cli.
"""
...
class CodeMapper:
    """Bidirectional mapping for API codes with case-insensitive lookup."""
    ...
    def get_code(self, name: str) -> int:
        if not name:
            raise ValueError(
                f"Invalid name: '{name}'. Must be one of: {self.options_str}"
            )
        code = self._name_to_code.get(name.lower())
        if code is None:
            raise ValueError(
                f"Unknown name '{name}'. Must be one of: {self.options_str}"
            )
        return code
