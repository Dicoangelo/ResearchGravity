...
╚══════════════════════════════════════════════════════════════════════════════╝""")
    ...
    for r in platform_events:
        report.append(
            f"  {r['platform']:<25} {r['event_count']:>10,} {pct(r['event_count'], total_events):>8}  {bar(r['event_count'], total_events)}"
        )
    ...
    report.append(
        f"  {'Platform':<25} {'Sessions':>10} {'Share':>8}  {'Avg Events/Session':>20}"
    )
    ...
    for r in platform_sessions:
        ev_count = next(
            (
                ...
                if p["platform"] == r["platform"]
            ),
            0,
        )
        avg_ev = ev_count / r["session_count"] if r["session_count"] else 0
        report.append(
            f"  {r['platform']:<25} {r['session_count']:>10,} {pct(r['session_count'], total_sessions):>8}  {avg_ev:>18.1f}"
        )
    ...
    for r in mode_dist:
        if r["platform"] != current_platform:
            ...
            plat_total = sum(
                x["cnt"] for x in mode_dist if x["platform"] == current_platform
            )
            report.append(f"\n  [{current_platform}] ({plat_total:,} events with mode)")
        report.append(
            f"    {r['cognitive_mode']:<20} {r['cnt']:>8,}  {pct(r['cnt'], plat_total):>7}  {bar(r['cnt'], plat_total, 30)}"
        )
    ...
    for r in embed_stats:
        report.append(
            f"  {r['platform']:<25} {r['embedded']:>8,} / {r['total']:>8,} events embedded ({pct(r['embedded'], r['total'])})"
        )
    total_embedded = sum(r["embedded"] for r in embed_stats)
    report.append(
        f"  {'TOTAL':<25} {total_embedded:>8,} / {total_events:>8,} events embedded ({pct(total_embedded, total_events)})"
    )
    ...
    report.append(
        "  Finding strongest semantic bridges between Claude and ChatGPT...\n"
    )
    ...
    if semantic_matches:
        report.append(
            f"  Found {len(semantic_matches)} cross-platform matches (similarity >= 0.65)\n"
        )
        ...
        for i, r in enumerate(semantic_matches[:15], 1):
            ...
            report.append(
                f"      Claude  [{r['claude_mode'] or '?':<12}]: {trunc(r['claude_text'], 90)}"
            )
            report.append(
                f"      ChatGPT [{r['chatgpt_mode'] or '?':<12}]: {trunc(r['chatgpt_text'], 90)}"
            )
        ...
        for r in semantic_matches:
            pair = (
                f"{r['claude_mode'] or 'unknown'} <-> {r['chatgpt_mode'] or 'unknown'}"
            )
    ...
    report.append(
        f"  {'Type':<30} {'Count':>6} {'Avg Conf':>10} {'Max Conf':>10} {'Avg Window':>12}"
    )
    report.append(f"  {'─' * 30} {'─' * 6} {'─' * 10} {'─' * 10} {'─' * 12}")
    for r in coherence_types:
        report.append(
            f"  {r['coherence_type']:<30} {r['cnt']:>6} {float(r['avg_conf']):>9.3f} {float(r['max_conf']):>9.3f} {float(r['avg_window']):>10.0f}s"
        )
    ...
    for r in conf_dist:
        report.append(
            f"    {r['bracket']:<30} {r['cnt']:>6}  {bar(r['cnt'], coherence_total, 30)}"
        )
    ...
    for i, r in enumerate(top_moments, 1):
        ...
        report.append(
            f"      Window: {r['time_window_s']}s  |  ID: {r['moment_id'][:40]}..."
        )
        if r["description"]:
    ...
    report.append(
        "  Identifying shared themes between platforms via semantic proximity...\n"
    )
    ...
    if topic_data:
        ...
        for r in topic_data[:30]:
            report.append(
                f"  {r['platform']:<20} {trunc(r['topics'] or '', 38):<40} {trunc(r['intent'] or '', 13):<15} {r['cnt']:>6}"
            )
    else:
        report.append(
            "  No light_layer topic data found. Analyzing via content keywords..."
        )
    ...
    for r in keyword_query:
        ...
        words = [
            w
            for w in r["preview_lower"].split()
    ...
    if len(platform_words) >= 2:
        ...
        for word in set().union(*(set(c.keys()) for c in platform_words.values())):
            ...
            if all(
                c >= 3 for c in counts.values()
            ):  # appears at least 3x on each platform
        ...
        for plat in all_platforms:
            unique = [
                (w, c)
                for w, c in platform_words[plat].most_common(200)
                if all(
                    platform_words[p].get(w, 0) < 2 for p in all_platforms if p != plat
                )
            ]
    ...
    report.append(
        f"  {'Platform':<20} {'Mode':<15} {'Events':>8} {'Sessions':>10} {'Avg Quality':>12} {'Avg Length':>12}"
    )
    report.append(f"  {'─' * 20} {'─' * 15} {'─' * 8} {'─' * 10} {'─' * 12} {'─' * 12}")
    for r in mode_comparison:
        avg_q = f"{float(r['avg_quality']):.3f}" if r["avg_quality"] else "N/A"
        avg_l = (
            f"{float(r['avg_content_length']):.0f}"
            if r["avg_content_length"]
            else "N/A"
        )
        report.append(
            f"  {r['platform']:<20} {r['cognitive_mode']:<15} {r['events']:>8,} {r['sessions']:>10,} {avg_q:>12} {avg_l:>12}"
        )
    ...
    if quality_dist:
        ...
        report.append(
            f"  {'Platform':<20} {'Mode':<15} {'P25':>8} {'P50':>8} {'P75':>8} {'P95':>8}"
        )
        report.append(
            f"  {'─' * 20} {'─' * 15} {'─' * 8} {'─' * 8} {'─' * 8} {'─' * 8}"
        )
        for r in quality_dist:
            report.append(
                f"  {r['platform']:<20} {r['cognitive_mode']:<15} {float(r['q25']):>7.3f} {float(r['q50']):>7.3f} {float(r['q75']):>7.3f} {float(r['q95']):>7.3f}"
            )
    ...
    if depth_analysis:
        ...
        report.append(
            f"  {'Platform':<20} {'Mode':<15} {'Deep(>1K)':>10} {'Medium':>10} {'Shallow(<200)':>14} {'Total':>8}"
        )
        report.append(
            f"  {'─' * 20} {'─' * 15} {'─' * 10} {'─' * 10} {'─' * 14} {'─' * 8}"
        )
        for r in depth_analysis:
            report.append(
                f"  {r['platform']:<20} {r['cognitive_mode']:<15} {r['deep_events']:>10,} {r['medium_events']:>10,} {r['shallow_events']:>14,} {r['total']:>8,}"
            )
    ...
    if xplat_matches and xplat_matches[0]["cnt"] > 0:
        report.append(
            f"  Cross-platform matches in dedicated table: {xplat_matches[0]['cnt']}"
        )
  ...
  KEY FINDINGS""")
    ...
    if semantic_matches:
        ...
        avg_sim = sum(float(r["similarity"]) for r in semantic_matches) / len(
            semantic_matches
        )
    ...
    if coherence_types:
        ...
        report.append(
            f"    - Dominant coherence type: {dominant_type['coherence_type']} ({dominant_type['cnt']} moments)"
        )

    # Platforms with most shared context
    if semantic_matches:
        high_sim_count = sum(
            1 for r in semantic_matches if float(r["similarity"]) >= 0.80
        )
...
""")
    ...
    report.append(
        f"  Database: ucw_cognitive | Platforms: {', '.join(p['platform'] for p in platform_events)}"
    )
    report.append(f"{'━' * 80}")
