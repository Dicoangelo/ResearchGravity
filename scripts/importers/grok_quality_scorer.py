...
def main():
    ...
    quality_scores = [
        ...
        for r in results
        if r["metrics"]["quality_score"] > 0
    ]

    print(f"\n{'=' * 60}")
    print("GROK QUALITY SCORING COMPLETE")
    print(f"{'=' * 60}")
    print(f"  Conversations scored: {len(results)}")
    print(f"  Total messages:       {total_msgs}")
    print(
        f"  Import recommended:   {importable} ({importable * 100 / len(results):.1f}%)"
    )
    if quality_scores:
        print(
            f"  Avg quality score:    {sum(quality_scores) / len(quality_scores):.3f}"
        )
    print()
    ...
    if deep_work:
        ...
        for r in deep_work[:15]:
            m = r["metrics"]
            print(
                f"  [{m['quality_score']:.3f}] [{m['purpose']:10s}] [{m['model']:30s}] {r['title'][:50]}"
            )
    print()
