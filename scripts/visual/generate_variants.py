...
# ── Comprehensive Source Context (fine-tuned from v2 run) ──────────────
SOURCE_CONTEXT = """
...
VARIANTS = [
    {
        "name": "architectural",
        "filename": "researchgravity_v3_architectural_4K.png",
        "caption": (
            "Detailed technical architecture blueprint of ResearchGravity v6.1 showing all 6 tiers "
            "with exact component names, file names (e.g., repl.py, concept_graph.py, archive_critic.py), "
            "and precise data flow arrows between every tier. "
            "CLEAN, STRUCTURED LAYOUT prioritizing completeness and technical accuracy. "
            "Show cross-cutting MCP Server on left side and REST API on right side as vertical strips. "
            "UCW Integration at the bottom with 3 semantic layers. "
            "Include ALL statistics as highlighted badges. "
            "Style: Technical blueprint with clear grid-like organization, minimal visual flourish, "
            "maximum information density. Every component labeled with its actual file name."
        ),
    },
    {
        "name": "hybrid",
        "filename": "researchgravity_v3_hybrid_4K.png",
        "caption": (
            "Architecture diagram of ResearchGravity v6.1 balancing technical detail with visual appeal. "
            "Show all 6 tiers with key components using visual metaphors: "
            "3D cylinders for databases (SQLite, Qdrant, sqlite-vec), "
            "brain icon for Intelligence Engine, shield icons for Critic Validation, "
            "node-edge graph for Knowledge Graph, funnel for Signal Capture. "
            "Include labeled data flow arrows between ALL tiers showing what data moves. "
            "Highlight key statistics (114+ sessions, 11,579 nodes, 27M+ tokens) in accent-colored badges. "
            "Cross-cutting MCP Server and REST API on sides. UCW at bottom with 3 layers. "
            "Style: Professional but visually engaging — suitable for both README and technical presentations. "
            "Use distinct pastel backgrounds per tier for visual grouping."
        ),
    },
    {
        "name": "infographic",
        "filename": "researchgravity_v3_infographic_4K.png",
        "caption": (
            "Data-rich infographic-style diagram of ResearchGravity v6.1 — Sovereign Research Intelligence. "
            "LARGE hero stats prominently displayed: 114+ Sessions, 2,530+ Findings, 8,935+ URLs, "
            "27M+ Tokens, 11,579 Graph Nodes, 3 Critics. "
            "Show the 6-tier architecture as a flowing data pipeline from Signal Capture through "
            "Intelligence Engine to Storage, Knowledge Graph, Critic Validation, and Visual Intelligence. "
            "Use icons extensively: magnifying glass for search, shield for critics, brain for AI, "
            "database cylinders for storage, graph nodes for knowledge graph. "
            "UCW at bottom as a foundation layer with 3 semantic layers shown as stacked translucent panels. "
            "Style: Modern infographic — bold typography, icon-heavy, stats-forward, presentation-ready. "
            "Think: the kind of diagram that gets shared on social media."
        ),
    },
    {
        "name": "dark-premium",
...
async def main():
    ...
    # Override to max quality for all variants
    config.apply_profile("max")
    config.cost_budget_per_session = 15.00  # Allow all 3 variants
    ...
    for i, variant in enumerate(VARIANTS):
        ...
        result = await pipeline.generate(
            ...
            aspect_ratio="16:9",
            quality="max",
            iterations=2,  # 2 rounds per variant (iter1 + final)
        ...
        print(f"  ✓ Generated: {result.get('png_path', 'unknown')}")
        print(f"  ✓ Resolution: {meta.get('resolution', '?')}")
        print(f"  ✓ Iterations: {meta.get('iterations_run', '?')}")
        print(f"  ✓ Cost: ${meta.get('estimated_cost_usd', 0):.4f}")
        print(f"  ✓ Time: {meta.get('elapsed_seconds', 0):.1f}s")

        # Print critic scores if available
