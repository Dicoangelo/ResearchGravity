...
class PackMetrics:
    ...
    def record_session(
        ...
        global_stats["avg_reduction_rate"] = (
            sum(all_rates) / len(all_rates) if all_rates else 0
        )
        ...
        for pack_id in packs_loaded:
            ...
            if pack_id in dq_scores:
                ...
                pack_stat["avg_dq_score"] = (
                    (current_avg * (count - 1)) + new_score
                ) / count
            ...
            for other_pack in packs_loaded:
                if other_pack != pack_id:
                    pack_stat["combined_with"][other_pack] = (
                        pack_stat["combined_with"].get(other_pack, 0) + 1
                    )
    ...
    def _calculate_cost_savings(
        self, tokens_saved: int, model: str = "sonnet"
    ) -> float:
    ...
    def _get_best_combinations(self, limit: int = 5) -> List[Dict]:
        ...
        for session in self.metrics["session_history"]:
            ...
            if len(packs) > 1:
                if packs not in combinations:
                    combinations[packs] = {"count": 0, "avg_savings": 0, "sessions": []}
        ...
        sorted_combos = sorted(
            combinations.items(), key=lambda x: x[1]["count"], reverse=True
        )
    ...
    def _get_daily_trend(self, days: int = 7) -> List[Dict]:
        ...
        for i in range(days):
            ...
            if date in self.metrics["daily_stats"]:
                trend.append({"date": date, **self.metrics["daily_stats"][date]})
            else:
                trend.append(
                    {
                        "date": date,
    ...
    def print_summary(self):
        ...
        for i, pack in enumerate(top_packs, 1):
            ...
            print(
                f"     Uses: {pack['times_selected']} | DQ: {pack['avg_dq_score']:.3f} | Consensus: {pack['avg_consensus_score']:.3f}"
            )
        ...
        for i, combo in enumerate(combos, 1):
            ...
            print(
                f"     Uses: {combo['uses']} | Avg Savings: {combo['avg_savings']:,.0f} tokens"
            )
        ...
        for day in trend:
            if day["sessions"] > 0:
                print(
                    f"  {day['date']}: {day['sessions']} sessions | {day['token_savings']:,} tokens | ${day['cost_savings']:.2f} saved"
                )
...
def main():
    ...
    parser.add_argument("--record", help="Record session metrics (provide session ID)")

    parser.add_argument("--packs", help="Comma-separated pack IDs loaded in session")

    parser.add_argument("--context", help="Session context")
    ...
    parser.add_argument("--pack-tokens", type=int, help="Actual tokens from packs")
    ...
    parser.add_argument(
        "--dashboard-data", action="store_true", help="Output dashboard JSON data"
    )
    ...
    if args.record:
        if not all([args.packs, args.context, args.baseline, args.pack_tokens]):
            print(
                "Error: --record requires --packs, --context, --baseline, --pack-tokens"
            )
            return
