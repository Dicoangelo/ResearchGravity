...
class EvidenceCritic(CriticBase):
    ...
    def __init__(
        self, sessions_dir: Optional[Path] = None, min_confidence: float = 0.7
    ):
    ...
    async def validate(self, target_id: str, **kwargs) -> ValidationResult:
        ...
        evidence = await self._gather_evidence(
            target_id, check_accessibility=check_accessibility
        )
    ...
    async def _gather_evidence(self, target_id: str, **kwargs) -> Dict[str, Any]:
        ...
        for finding in evidence["findings"]:
            sources = finding.get("sources", []) or finding.get("evidence", {}).get(
                "sources", []
            )
            if sources:
    ...
    def _calculate_confidence(
        ...
        # Finding citations (30%)
        total_findings = (
            metrics["findings_with_sources"] + metrics["findings_without_sources"]
        )
        if total_findings > 0:
...
async def main():
    ...
    if result.issues:
        ...
        for issue in result.issues:
            ...
            print(
                f"  {icon.get(issue.severity.value, '•')} [{issue.code}] {issue.message}"
            )
