...
class TestPioneerMode:
    ...
    def test_pioneer_weights_applied(self):
        ...
        assert (
            PIONEER_DQ_WEIGHTS["correctness"] < PRECISION_DQ_WEIGHTS["correctness"]
        ...
        assert abs(total - 1.0) < 0.01, (
            f"Pioneer weights should sum to 1.0, got {total}"
        )
    ...
    def test_trust_context_weights_applied(self):
        ...
        assert abs(total - 1.0) < 0.01, (
            f"Trust context weights should sum to 1.0, got {total}"
        )
...
class TestPioneerAutoDetection:
    ...
    async def test_pioneer_auto_detection_sparse_tier1(self, sample_search_results):
        ...
        if len(context.tier1_results) < 3:
            pioneer_signals.append(
                f"sparse Tier 1 results ({len(context.tier1_results)} found)"
            )
    ...
    def test_pioneer_auto_detection_recent_sources(self, sample_search_results):
        ...
        if recent_ratio > 0.6:
            pioneer_signals.append(
                f"mostly recent sources ({recent_count}/{len(results)} < 14 days)"
            )
...
class TestTrustContext:
    ...
    def test_trusted_context_creates_tier1(self, user_context):
        ...
        from cpb.search_layer import (
            create_trusted_user_context,
            SourceTier,
            SourceCategory,
        )
        ...
        assert trusted.category == SourceCategory.INTERNAL, (
            "Should be internal category"
        )
...
class TestDeepResearch:
    ...
    def test_deep_research_result_conversion(self, sample_deep_research_result):
        ...
        assert main_result.base_relevance >= 0.9, (
            "Main result should have high relevance"
        )
