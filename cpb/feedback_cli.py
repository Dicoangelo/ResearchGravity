...
def show_stats():
    ...
    print(
        f"Average rating:    {avg_rating:.2f}/5.0 {'★' * round(avg_rating)}{'☆' * (5 - round(avg_rating))}"
    )
...
def interactive_feedback():
    ...
    while True:
        ...
        verified_claims = (
            [c.strip() for c in verified_input.split(",")] if verified_input else None
        )

        # Optional false claims
        false_input = input("False claims (comma-separated, or blank): ").strip()
        false_claims = (
            [c.strip() for c in false_input.split(",")] if false_input else None
        )
...
Examples:
    ...
    parser.add_argument(
        "--verified-claims", nargs="*", default=None, help="Claims verified as true"
    )
    parser.add_argument(
        "--false-claims", nargs="*", default=None, help="Claims identified as false"
    )

    parser.add_argument(
        "--list", "-l", action="store_true", help="List recent feedback entries"
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of entries to show in list"
    )
    parser.add_argument(
        "--stats", "-s", action="store_true", help="Show feedback statistics"
    )
    parser.add_argument(
        "--export", "-e", type=str, metavar="FILE", help="Export feedback to JSON file"
    )
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="Interactive feedback mode"
    )
