...
def get_precision_agent_prompts(
    ...
    for agent in PRECISION_AGENT_PERSONAS:
        ...
        if context:
            user_prompt_parts.append(
                f"## CONTEXT (from ResearchGravity)\n{context[:15000]}"
            )
...
def validate_precision_config(config: PrecisionConfig) -> List[str]:
    ...
    if config.dq_threshold < 0.90:
        warnings.append(
            f"DQ threshold ({config.dq_threshold}) below 0.90 - not truly 'precision' mode"
        )

    if config.ace_config.agent_count < 5:
        warnings.append(
            f"Agent count ({config.ace_config.agent_count}) low for precision consensus"
        )

    if config.max_retries < 3:
        warnings.append(
            f"Max retries ({config.max_retries}) may not be enough to hit 0.95 threshold"
        )

    if not config.critic_validation:
        warnings.append(
            "Critic validation disabled - precision mode requires critic verification"
        )

    if not config.force_cascade:
        warnings.append(
            "Force cascade disabled - precision mode should use full pipeline"
        )
