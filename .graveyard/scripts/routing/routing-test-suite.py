...
REGRESSION_QUERIES = [
    ...
    {
        "query": "Write a React hook for API calls with error handling",
        "expected": "sonnet",
        "complexity_range": [0.3, 0.7],
    },
    {
        "query": "Refactor this code to use dependency injection",
        "expected": "sonnet",
        "complexity_range": [0.3, 0.7],
    },
    {
        "query": "Create SQL query to join 3 tables and aggregate results",
        "expected": "sonnet",
        "complexity_range": [0.3, 0.7],
    },
    # Opus-optimal (complex, architectural)
    {
        "query": "Design a distributed caching system with consistency guarantees",
        "expected": "opus",
        "complexity_range": [0.7, 1.0],
    },
    {
        "query": "Architect a microservices system for real-time data processing at scale",
        "expected": "opus",
        "complexity_range": [0.7, 1.0],
    },
    {
        "query": "Explain trade-offs between CAP theorem choices for financial transactions",
        "expected": "opus",
        "complexity_range": [0.7, 1.0],
    },
    {
        "query": "Design fault-tolerant distributed queue with exactly-once delivery",
        "expected": "opus",
        "complexity_range": [0.7, 1.0],
    },
    {
        "query": "Analyze security vulnerabilities in OAuth 2.0 implementations",
        "expected": "opus",
        "complexity_range": [0.7, 1.0],
    },
]
...
class RoutingTestSuite:
    ...
    def run_regression_tests(self) -> Dict:
        ...
        for i, test_case in enumerate(REGRESSION_QUERIES, 1):
            ...
            try:
                ...
                complexity_valid = (
                    complexity_range[0] <= complexity <= complexity_range[1]
                )

                if passed and complexity_valid:
                    print(
                        f"  ✓ PASS: {actual_model} (complexity: {complexity:.2f}, DQ: {dq_score:.2f}, {latency_ms:.0f}ms)"
                    )
                    correct += 1
                else:
                    if not passed:
                    ...
                    if not complexity_valid:
                        print(
                            f"  ⚠ Complexity {complexity:.2f} outside range {complexity_range}"
                        )
        ...
        if BASELINES_FILE.exists():
            try:
                ...
                if haiku_max == sonnet_min and sonnet_max == opus_min:
                    ...
                    print(
                        f"  ⚠ Threshold gaps detected: haiku({haiku_max}) → sonnet({sonnet_min}-{sonnet_max}) → opus({opus_min})"
                    )
    ...
    def generate_report(self, output_format: str = "text") -> str:
        ...
        if "regression" in self.results and self.results["regression"]:
            ...
            lines.append(
                f"📊 Regression Tests: {reg['passed']}/{reg['total']} passed ({reg['accuracy']:.1%})"
            )
        ...
        if "performance" in self.results and "latency" in self.results["performance"]:
            ...
            lines.append(
                f"⏱️  Routing Latency: p95={perf['p95_ms']:.1f}ms (target: <50ms) {'✓' if perf['target_met'] else '✗'}"
            )
        ...
        # Overall status
        all_pass = (
            self.results.get("regression", {}).get("accuracy", 0) >= 0.80
            and self.results.get("performance", {})
            .get("latency", {})
            .get("target_met", False)
            and sum(1 for v in self.results.get("validation", {}).values() if v)
            == len(self.results.get("validation", {}))
        )
...
def main():
    parser = argparse.ArgumentParser(description="Routing system test suite")
    ...
    perf_parser = subparsers.add_parser(
        "performance", help="Run performance tests only"
    )
    ...
    if args.command == "all":
        ...
        all_pass = (
            suite.results.get("regression", {}).get("accuracy", 0) >= 0.80
            and suite.results.get("performance", {})
            .get("latency", {})
            .get("target_met", False)
            and sum(1 for v in suite.results.get("validation", {}).values() if v)
            == len(suite.results.get("validation", {}))
        )
